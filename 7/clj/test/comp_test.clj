(ns comp-test (:require [comp :refer [run amplify-signal get-max-signal]]
                        [clojure.test :refer :all]))

(deftest day5
  (is (= (run [3 9 8 9 10 9 4 9 99 -1 8] [8]) [1]))
  (is (= (run [3 9 8 9 10 9 4 9 99 -1 8] [7]) [0]))
  (is (= (run [3,9,7,9,10,9,4,9,99,-1,8] [7]) [1]))
  (is (= (run [3,9,7,9,10,9,4,9,99,-1,8] [9]) [0]))
  (is (= (run [3,3,1108,-1,8,3,4,3,99] [8]) [1]))
  (is (= (run [3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9] [8]) [1]))
  (is (= (run [3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9] [0]) [0]))
  (is (= (run [3,3,1105,-1,9,1101,0,0,12,4,12,99,1] [8]) [1])))

(deftest day-5-big-example
  (is (= (run [3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31
